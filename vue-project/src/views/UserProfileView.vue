    ...
    <div class="container-fluid" v-if="profile">
      <div class="editorial-layout">
        ...
        <aside class="identity-sidebar">
          ...
          <section class="identity-body">
            <div class="name-block">
              <h1 class="user-nickname">{{ profile.nickname || profile.username }}</h1>
              <span class="user-email">{{ profile.email }}</span>
            </div>
            <div class="intro-block">
              <p class="user-intro">{{ profile.intro || 'A silent observer of the moving image.' }}</p>
            </div>
          </section>

          <section class="social-data">
            <div class="data-row" @click="uiStore.showFollowList(profile.id, 'followers')">
              <span class="data-label">팔로워</span>
              <span class="data-dot"></span>
              <span class="data-value">{{ profile.follower_count ?? 0 }}</span>
            </div>
            <div class="data-row" @click="uiStore.showFollowList(profile.id, 'following')">
              <span class="data-label">팔로잉</span>
              <span class="data-dot"></span>
              <span class="data-value">{{ profile.following_count ?? 0 }}</span>
            </div>
          </section>

          <footer class="action-zone" v-if="userStore.isLogin && !isMe">
            <button 
              class="btn-follow-minimal" 
              :class="{ 'is-following': isFollowing }"
              @click="toggleFollowHandler"
            >
        ...
        <main class="records-feed">
          ...
          <!-- Navigation Tabs (Modern Typography Style) -->
          ...
          <div class="records-container">
            ...
            <!-- A. Movie Grids (Collection & Rating) -->
            ...
            <!-- B. List Contents (Comments & Articles) -->
                  ...
                  <div v-for="r in paginatedComments" :key="r.id" class="stream-item" @click="goMovie(r.movie)">
                    ...
                    <div class="stream-body">
                      ...
                      <div class="stream-content">
                        <div v-if="!isMe && r.is_spoiler && !visibleSpoilers[r.id]" class="spoiler-box">
                          <p>스포일러 주의</p>
                          <button @click.stop="visibleSpoilers[r.id] = true" class="btn-reveal">보기</button>
                        </div>
                        <p v-else class="quote-text">"{{ r.content || '...' }}"</p>
                      </div>
                    </div>
                  </div>
                </div>
...
// Cloud Run 백엔드 URL (프로덕션용)
const BACKEND_BASE_URL = 'https://youngcha-backend-101935470358.asia-northeast3.run.app'
...
const tabs = computed(() => [
  { id: 'collection', label: 'COLLECTION', count: profile.value?.like_movies?.length || 0 },
  { id: 'rating', label: 'RATINGS', count: profile.value?.rating_count || 0 },
  { id: 'comment', label: 'COMMENTS', count: profile.value?.comment_count || 0 },
  { id: 'articles', label: 'ARTICLES', count: profile.value?.articles?.length || 0 },
])

const paginatedCollection = computed(() => {
  const list = profile.value?.like_movies || []
  return list.slice((collectionPage.value - 1) * itemsPerPageMovies, collectionPage.value * itemsPerPageMovies)
})
const collectionTotalPages = computed(() => Math.ceil((profile.value?.like_movies?.length || 0) / itemsPerPageMovies))
...
const paginatedArticles = computed(() => {
  const list = profile.value?.articles || []
  return list.slice((articlesPage.value - 1) * itemsPerPageList, articlesPage.value * itemsPerPageList)
})
const articlesTotalPages = computed(() => Math.ceil((profile.value?.articles?.length || 0) / itemsPerPageList))
...
//   axios({
//     method: 'get',
//     url: `${API_URL}/api/accounts/users/${userId}/`,
//     headers: userStore.token ? { Authorization: `Token ${userStore.token}` } : {},
//   })
//     .then((res) => {
...
//       }
//       userStore.fetchUserRatings(userId)
//       userStore.fetchUserReviews(userId)
//     })
//     .catch((err) => {
//       console.error(err)
//       profile.value = null
...
const loadProfile = () => {
  const userId = Number(route.params.id)
  api.get(`/api/accounts/users/${userId}/`, {
    headers: userStore.token ? { Authorization: `Token ${userStore.token}` } : {},
  })
    .then((res) => {
      profile.value = res.data
...
//   axios({
//     method: 'post',
//     url: `${API_URL}/api/accounts/follow/${route.params.id}/`,
//     headers: { Authorization: `Token ${userStore.token}` },
//   })
//     .then((res) => {
//       if (profile.value) {
//         profile.value.follower_count = res.data.follower_count
//       }
//       isFollowing.value = res.data.is_following
//     })
//     .catch((err) => {
...
const toggleFollowHandler = () => {
  ...
  api.post(`/api/accounts/follow/${route.params.id}/`, null, {
    headers: { Authorization: `Token ${userStore.token}` },
  })
    .then((res) => {
      if (profile.value) {
...
const isMe = computed(() => userStore.profile && profile.value && userStore.profile.id === profile.value.id)
...
// 기존에 API_URL 쓰던 부분들만 BASE_URL로 교체
const profileImageUrl = computed(() => {
  const path = profile.value?.profile_image
  if (!path) return `${BACKEND_BASE_URL}/media/images/no_actor_image.png`
...
.btn-follow-minimal:hover {
  background: #409E99;
}

.btn-follow-minimal.is-following {
  background: transparent;
  color: #1a1a1a;
  border: 1px solid #1a1a1a;
}

.btn-follow-minimal.is-following:hover {
  border-color: #ff4d4d;
  color: #ff4d4d;
}
...
.records-feed { 
  flex-grow: 1; 
  min-height: 1000px; /* 피드 영역 자체의 최소 높이 확보 */
}

.records-container {
  min-height: 800px; /* 콘텐츠 영역의 최소 높이를 상향 조정 */
  position: relative;
}
...
.editorial-layout {
  ...
  padding-top: 100px;
  padding-bottom: 120px;
  min-height: 1200px; /* 레이아웃 전체의 최소 높이를 1200px 이상으로 고정 */
}
...
/* Content Styles - Filmic Specimen Redesign */
.editorial-grid {
  display: grid;
  grid-template-columns: repeat(3, 1fr); /* 3열로 조정하여 세로 포스터 공간 확보 */
  gap: 40px;
}
...
.poster-box {
  width: 100%;
  aspect-ratio: 2 / 3; /* 세로 비율 복구 */
  overflow: hidden;
  border-radius: 2px;
  background: #f0f0f0;
  position: relative; /* 평점 뱃지 고정을 위해 추가 */
  box-shadow: 10px 10px 0px rgba(0,0,0,0.03); /* 초기 그림자 */
  border: 1px solid #eee;
}
...
.rating-overlay {
  ...
  z-index: 2;
  line-height: 1;
  font-family: 'Inter', sans-serif; /* MyPage와 동일한 폰트 강제 적용 */
}
...
/* Stream Items */
.stream-item {
  display: flex; gap: 25px; padding: 20px 0; border-bottom: 1px solid #eee; cursor: pointer;
}
...
.stream-body { 
  ...
  display: flex; 
  flex-direction: column; 
  justify-content: center; /* 다시 중앙 정렬로 변경하여 콤팩트함 유지 */
}
.stream-header { 
  display: flex; 
  justify-content: space-between; 
...
.stream-title { font-size: 1.25rem; font-weight: 900; letter-spacing: -0.5px; }
.stream-score { font-size: 0.85rem; font-weight: 800; color: #409E99; }

.stream-content {
  min-height: 45px; /* 높이 축소 */
  display: flex;
  align-items: center;
}

.quote-text {
  font-size: 1.05rem; line-height: 1.6; color: #555; font-style: italic; margin: 0;
  border-left: 2px solid #eee; padding-left: 15px;
  width: 100%;
}
...
/* Article Minimal */
.article-minimal {
  padding: 40px 0; border-bottom: 1px solid #1a1a1a; cursor: pointer;
  transition: 0.3s;
}
...
.empty-state {
  padding: 100px 0; text-align: center; font-size: 0.9rem; font-weight: 900; color: #ccc; letter-spacing: 5px;
}
